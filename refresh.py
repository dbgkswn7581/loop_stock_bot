...
def update_stock(company, time):
  ...
  c = conn.cursor()
  c.execute('UPDATE {} SET value=%s WHERE num=%s'.format(name), (int(value), num))
  c.execute('UPDATE {} SET date=%s WHERE num=%s'.format(name), (date, num))
  c.execute('UPDATE {} SET clock=%s WHERE num=%s'.format(name), (clock, num))
  conn.commit()
  c.close()
  conn.close()
